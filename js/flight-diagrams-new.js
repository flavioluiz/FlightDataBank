...
document.addEventListener('DOMContentLoaded', async function() {
    console.log('DOM loaded - initializing flight diagrams (new version)');
    try {
...
async function loadDiagramConfig() {
    try {
        ...
        console.log('Diagram configuration loaded successfully:', diagramConfig);
    } catch (error) {
...
function initializeControls() {
    ...
    if (!aircraftData || !Array.isArray(aircraftData) || aircraftData.length === 0) {
        console.error('Aircraft data not loaded properly');
        return;
    }
    ...
    if (controls.diagramType) {
        ...
        controls.diagramType.value = Object.keys(diagramConfig.diagrams)[0];
    ...
    if (controls.colorGroup) {
        ...
        controls.colorGroup.value = Object.keys(diagramConfig.colorGroups)[0];
...
function updateTrendlineControls() {
    ...
    if (!diagramType || !diagramConfig?.diagrams?.[diagramType]) {
        console.warn('Invalid diagram type for trendline controls:', diagramType);
        return;
    }
